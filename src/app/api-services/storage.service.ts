...
export class StorageService {
  ...
  //SERVICIO PARA DAR EL TIPO DE USUARIO
  public async setUserType(userType: 'therapist' | 'patient') {
...
//SERVICIO PARA DAR EL USUARIO ACTUAL
  ...
  async updateCurrentUser(newUserProperties: any): Promise<any> {
    ...
    // console.log("Usuario actual en Storage: ", currentUser);

    // Si no hay una nextAppointment activa quita el valor anterior
    if(!newUserProperties.nextAppointment){
      currentUser['nextAppointment'] = null;
    }

    if(!newUserProperties.appointments){
      currentUser['appointments'] = [];
    } else if(!newUserProperties.appointments.length){
      currentUser['appointments'] = [];
    }
  ...
  //servicio para dar el id del chat actual REVISAR EL SWAGGER PARA VER EL TIPO DE DATO
  async setCurrentChatId(chatId: string) {
    return await this.storage.set('currentChatId', chatId)
  }
