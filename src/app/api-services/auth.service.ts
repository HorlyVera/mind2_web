...
export class AuthService {
  ...
  public async signupUser(user: any) {
    return await this.http.post(`/auth/signup`, user).toPromise()
  }

  public async sendVerificationEmail(email: any) {
    return await this.http.post(`/auth/send-verification/${email}`, email).toPromise()
  }
  ...
  public async loginUser(data) {
    const newSession = await this.http.post<any>(`/auth/login`, data).toPromise()
